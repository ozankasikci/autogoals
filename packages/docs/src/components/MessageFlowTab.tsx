...
const flows: Flow[] = [
  ...
  {
    id: "add-rule",
    title: "Add/Edit a Rule",
    description: "How a user-defined rule propagates through the system to a running agent",
    steps: [
      {
        component: "User",
        action: "Types a rule in the Rules panel",
        detail: "User writes a constraint like 'Never delete files without confirmation' and clicks Add",
        componentColor: "User",
      },
      {
        component: "Dashboard",
        action: "ADD_RULE mutation",
        detail: "Apollo Client sends addRule mutation with projectId and rule content",
        file: "dashboard/src/graphql/operations.ts",
        componentColor: "Dashboard",
      },
      {
        component: "API",
        action: "resolvers.addRule()",
        detail: "Inserts the rule into the rules table via store.addRule(content)",
        file: "api/src/schema/resolvers.ts",
        componentColor: "API",
      },
      {
        component: "API",
        action: "Check if agent is running",
        detail: "Resolver checks agentManager.isRunning(projectId) to decide whether to notify the live agent",
        file: "api/src/schema/resolvers.ts",
        componentColor: "API",
      },
      {
        component: "AgentManager",
        action: "sendMessage('[System] Rules updated...')",
        detail: "Full rules list is sent as a [System] message: 'Project rules have been updated. Current rules: ...'",
        file: "api/src/schema/resolvers.ts",
        componentColor: "AgentManager",
      },
      {
        component: "Agent",
        action: "Acknowledges new constraint",
        detail: "Agent receives the system message with updated rules and integrates them into its working context",
        componentColor: "Agent",
      },
      {
        component: "Dashboard",
        action: "Refetch shows updated rule",
        detail: "Dashboard query refetch shows the new rule in the Rules panel",
  ...
  {
    id: "rules-guide",
    title: "How Rules Guide Execution",
    description: "How rules are composed into the system prompt and constrain agent behavior during goal execution",
    steps: [
      {
        component: "Agent",
        action: "Starts working on a goal",
        detail: "Agent picks up the next pending goal from the execution queue",
        componentColor: "Agent",
      },
      {
        component: "AgentManager",
        action: "Load rules from DB",
        detail: "Calls store.getRules() to fetch all user-defined rules for this project",
        file: "api/src/schema/resolvers.ts",
        componentColor: "AgentManager",
      },
      {
        component: "AgentManager",
        action: "Load goal details",
        detail: "Loads goal name, description, approach, and acceptance criteria from the goals table",
        file: "api/src/schema/resolvers.ts",
        componentColor: "AgentManager",
      },
      {
        component: "AgentManager",
        action: "Compose system prompt",
        detail: 'Builds prompt: "RULES (you MUST follow ALL):\\n- rule 1\\n- rule 2\\n\\nGOAL: [name]\\nCRITERIA: [list]"',
        file: "api/src/schema/resolvers.ts",
        componentColor: "AgentManager",
      },
      {
        component: "Agent",
        action: "Receives prompt with rules as guardrails",
        detail: "Claude Code receives the composed system prompt containing rules before any goal context",
        componentColor: "Agent",
      },
      {
        component: "Agent",
        action: "Implements goal while complying with rules",
        detail: "Agent executes the goal's approach, checking each action against the rules constraints",
        componentColor: "Agent",
      },
      {
        component: "Agent",
        action: "If conflict: flags it in chat",
        detail: "When a goal requirement conflicts with a rule, the agent reports the conflict in chat instead of breaking the rule",
        componentColor: "Agent",
      },
    ],
  },
