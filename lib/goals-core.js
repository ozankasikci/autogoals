 ...
 * @throws {Error} - If goal ID not found in state
 */
export function updateGoalStatus(goalId, status, state) {
  // Validate goal exists in state
  if (!state.goals_status.hasOwnProperty(goalId)) {
    throw new Error(`Goal '${goalId}' not found in state`);
  }
